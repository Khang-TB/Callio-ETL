...
from callio_etl.api import CallioAPI
from callio_etl.bigquery_service import BigQueryService
from callio_etl.checkpoints import CheckpointStore, UpdateLogBuffer
from callio_etl.config import PipelineConfig
from callio_etl.logging_utils import configure_logging, track_progress
from callio_etl.utils import (
    compute_row_hash,
...
class CallioETLRunner:
    ...
    def run_customer_for_tenant(
        self, tenant: str
    ) -> Tuple[Tuple[Optional[date], Optional[date]], int, Optional[int]]:
        table = "customer"
    ...
    def merge_customer_window(self, start: date, end: date) -> None:
        self.bq.ensure_table_schema_customer()
        self.bq.ensure_table_schema_customer_staging()
    ...
    def _run_customer_all_tenants(self, schedule_label: str) -> None:
        ...
        window_min: Optional[date]
        window_max: Optional[date]
        window_min, window_max = None, None
        staged_stats: Dict[str, Tuple[int, Optional[int]]] = {}
        for account in track_progress(
            self.config.api.accounts,
            ...
            try:
                ...
                staged_stats[account.tenant] = (rows, max_update)
        ...
        if window_min and window_max:
            try:
                ...
                for tenant, stats in staged_stats.items():
                    rows_staged, max_update = stats
                    if max_update is not None:
                        self.checkpoints.set_checkpoint("customer", tenant, max_update)
                        self.log_buffer.add(tenant, "customer", rows_staged, max_update, "MERGED")
                        self.logger.info(
    ...
    def run_fact_staff_daily_pk_refresh(self) -> None:
        ...
        DECLARE part_min DATE      DEFAULT DATE_SUB(part_max, INTERVAL {window_days} DAY);
        ...
        USING (
          ...
          assigned AS (
            ...
            WHERE tenant = 'PK'                                           -- ✅ PK only
              AND assignedTime IS NOT NULL
              AND DATE(TIMESTAMP_MILLIS(assignedTime), 'Asia/Ho_Chi_Minh') BETWEEN part_min AND part_max
            GROUP BY Ngay, MaNV_id
          ),
